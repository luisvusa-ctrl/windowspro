...
int GetMaxClip(std::string name) {
	...
	// Armas de 30 balas (Padrão)
	// Verifica M4A4 explicitamente ou assume 30 se não for a Silencer
	...
	// IMPORTANTE: P2000 e P250 (13 balas)
	// O nome interno pode ser 'hkp2000' ou apenas 'p2000'
	...
	// Heavy Pistols e Shotguns curtas
					...
					if ((screenPos.x != 0.0f && screenPos.y != 0.0f) && (headScreenPos.x != 0.0f && headScreenPos.y != 0.0f))
					{
						...
						if (Globals::espAmmoBarEnabled)
						{
							...
							if (clippingWeapon != 0)
							{
								...
								std::string wName = entity.weaponName;

								// CONVERTER PARA MINÚSCULO (CORREÇÃO DE SEGURANÇA)
								// Adicione #include <algorithm> no topo do arquivo se der erro aqui
								std::transform(wName.begin(), wName.end(), wName.begin(), ::tolower);

								bool isKnifeOrUtility = (
									wName.find("knife") != std::string::npos ||
									wName.find("bayonet") != std::string::npos ||
									wName.find("karambit") != std::string::npos ||
									wName.find("taser") != std::string::npos ||
									wName.find("grenade") != std::string::npos ||
									wName.find("flashbang") != std::string::npos ||
									wName.find("c4") != std::string::npos
									);

								int maxAmmo = GetMaxClip(wName);

								// Só desenha se a munição for válida e tiver um pente máximo definido (> 0)
								if (currentAmmo >= 0 && currentAmmo < 500 && !isKnifeOrUtility && maxAmmo > 0)
								{
									// Garante que currentAmmo nunca seja maior que maxAmmo no calculo visual
									int displayAmmo = (currentAmmo > maxAmmo) ? maxAmmo : currentAmmo;

									float ammoPerc = (float)displayAmmo / (float)maxAmmo;

									// Travas de segurança
									if (ammoPerc > 1.0f) ammoPerc = 1.0f;
									if (ammoPerc < 0.0f) ammoPerc = 0.0f;

									float barHeight = 4.0f;
									float gap = 3.0f;

									float barLeftX = topLeft.x;
									float barRightX = bottomRight.x;
									float topY = bottomRight.y + gap;
									float bottomY = topY + barHeight;

									// Fundo
									drawList->AddRectFilled({ barLeftX, topY }, { barRightX, bottomY }, IM_COL32(0, 0, 0, 255));

									// Barra
									float barWidth = (barRightX - barLeftX) * ammoPerc;
									ImU32 ammoColor = IM_COL32(255, 190, 0, 255);
									if (ammoPerc <= 0.2f) ammoColor = IM_COL32(255, 50, 50, 255);

									drawList->AddRectFilled({ barLeftX, topY }, { barLeftX + barWidth, bottomY }, ammoColor);

									// Borda
									drawList->AddRect({ barLeftX, topY }, { barRightX, bottomY }, IM_COL32(0, 0, 0, 255));

									// Texto numérico (Fonte 2 para texto normal)
									if (currentAmmo < maxAmmo)
									{
										std::string ammoText = std::to_string(currentAmmo);
										drawList->AddText(imguiIo.Fonts->Fonts[2], 12.0f, { barLeftX + barWidth, bottomY }, IM_COL32(255, 255, 255, 255), ammoText.c_str());
									}
								}
							}
						}
